...
model_type = input('Enter the model type & technique: 0 for BiLSTM model without TF, 1 for BiLSTM model with TF, 2 for BiLSTM model with TF & FT')
model_type = int(model_type)

if model_type == 0:
    print('BiLSTM model without TF selected.')
    embedding_path = '/embeddings/target_glove.txt'
    model_type = '256_bilstm'
elif model_type == 1:
    print('BiLSTM model with TF selected.')
    embedding_path = '/embeddings/source_glove.txt'
    model_type = 'tf'
elif model_type == 2:
    print('BiLSTM model with TF & FT selected.')
    embedding_path = '/embeddings/tf_finetuned_glove.txt'
    model_type = 'tf_finetune'
else:
...
overfitCallback = [EarlyStopping(monitor='val_loss', min_delta=0, patience=10),
         ModelCheckpoint(filepath='/models/{0}/best_model.h5'.format(model_type), monitor='val_loss', save_best_only=True)]
...
model.load_weights('/models/{0}/best_model.h5'.format(model_type))
...
plt.legend()
#plt.show()
plt.savefig('/models/{0}/{0}_loss.png'.format(model_type))
...
plt.legend()
#plt.show()
plt.savefig('/models/{0}/{0}_accuracy.png'.format(model_type))

# Save history
with open('/models/{0}/{0}_trainHistoryDict'.format(model_type), 'wb') as file_pi:
